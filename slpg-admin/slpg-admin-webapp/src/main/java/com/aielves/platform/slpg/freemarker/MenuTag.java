package com.aielves.platform.slpg.freemarker;
...
public class MenuTag implements TemplateDirectiveModel {
    ...
    public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody directiveBody)
            throws IOException {
        try {
            RbacResourceService rbacResourceService = SpringUtils.getBean(RbacResourceService.class);
            List<RbacResource> level1s = rbacResourceService.findByCnd(new SQLCnd().eq("pid", 0).orderby("orderno", SortBy.A));
            StringBuffer buffer = new StringBuffer("");
            for (RbacResource level1 : level1s) {
                List<RbacResource> level2s = rbacResourceService.findByCnd(new SQLCnd().eq("pid", level1.getId()).orderby("orderno", SortBy.A));
                if (level2s.isEmpty()) {
                    continue;
                }
                // 大模块栏目开始
                ...
                // 大模块栏目结束
                for (RbacResource level2 : level2s) {
                    // 小模块栏目开始
                    ...
                    List<RbacResource> level3s = rbacResourceService.findByCnd(new SQLCnd().eq("pid", level2.getId()).orderby("orderno", SortBy.A));
                    for (RbacResource level3 : level3s) {
                        // 节点块资源开始
                        buffer.append("<li class=\"sidebar-nav-link\">");
